...
ObjectStoreStub.prototype.storeFile = function( containerName, localFile, callback) {
	...
	objectStore.storageClient.getContainer(containerName, function (err, container) {
		...
		writeStream.on('error', function(err) {
			console.log('Error while trying to push csv file ('+err.href+') to Object Storage (putFile operation),  err='+JSON.stringify(err));
		});
		
		writeStream.on('success', function(file) {
			console.log('File was uploaded to Object storage');
			
			if ( callback) {
				callback( objectStore, containerName, remoteFileName);
			}
		});
...
ObjectStoreStub.prototype.retrieveFile = function( containerName, fileName, localFile, callback) {
	var options = {
		container: containerName,
		remote: fileName+"11112"
	};

	var stream = this.storageClient.download(options, function (err, res) {
		if (err) {
			callback(null,err);
		} else {
			//console.log(res);
